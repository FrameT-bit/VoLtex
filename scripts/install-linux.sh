...
copy_app() {
  ...
  case "$install_real" in
    "/"|"$HOME"|"$HOME/.local"|"$HOME/.local/share"|"$HOME/.local/opt")
...
install_python_environment() {
  ...
  "$INSTALL_DIR/.venv/bin/python" -m pip install -r "$INSTALL_DIR/requirements.txt" </dev/null || {
    print_dependency_help
...
install_desktop_integration() {
  ...
  chmod +x "$INSTALL_DIR/run-voltex"
  chmod +x "$INSTALL_DIR/install" "$INSTALL_DIR/install.sh" "$INSTALL_DIR/uninstall.sh" 2>/dev/null || true
  chmod +x "$INSTALL_DIR/scripts/install-linux.sh" "$INSTALL_DIR/scripts/uninstall-linux.sh" 2>/dev/null || true
